...
echo ================================================
echo  gc2xy - Build (Node.js portable)
echo  Creates portable .dist folder with Node + tsx.
echo ================================================
echo.
...
:: -- Step 2: Install production dependencies inside .dist --
...
:: -- Step 3: Copy node.exe to .dist (renamed to "node" no extension) --
echo [3/5] Copying Node.js runtime...
set NODEPATH=
...
if exist "!NODEPATH!" (
    ...
    echo [INFO]   ^> node.exe copied to .dist\node
) else (
    echo [WARN]   ^> node.exe not found, service will use PATH
)
...
echo.
echo ================================================
echo  Build successful ^(Node.js portable^)
echo ================================================
echo.
...
echo     sc start gc2xy-mock
echo.
echo     sc create gc2xy-hybrid binPath= "\"%%CD%%\.dist\service-hybrid.exe\"" start= auto
echo     sc start gc2xy-hybrid
echo.
echo     sc create gc2xy-proxy binPath= "\"%%CD%%\.dist\service-proxy.exe\"" start= auto
echo     sc start gc2xy-proxy
echo.
echo     sc stop gc2xy-mock ^&^& sc delete gc2xy-mock
echo.
echo   Notes:
echo     - node binary must be in same directory as service-*.exe
echo     - Or have Node.js installed and available in PATH

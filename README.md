...
- **Wallpaper switcher** in Proxy Configuration: None / Bing (daily wallpaper) / Wallhaven (random SFW from monthly top list) / **AI (FreeGen)** (AI-generated from prompt via `freegen.app`, no key required). All cached for 1 hour. Selection persisted in `localStorage` and `.config/config.json` (`wallpaper` field).

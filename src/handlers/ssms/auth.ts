...
export function isSSMS(headers: Record<string, string>): boolean {
 ...
 */
export async function handleSSMSChat(req: HandlerInput): Promise<HandlerResult> {
  const { headers, method, url } = req;
  if (!isSSMS(headers)) return { handled: false };
  ...
  console.log(`[SSMS] Delegating ${req.method} ${req.url} to VS handler`);
